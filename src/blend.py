...
def blend_predictions(
    ...
    Blending strategy:
        - High confidence market data: weighted blend (default 0.6/0.4)
        - Low confidence market data: conservative blend (0.85/0.15)
    ...
    Args:
        ...
        market_signals: DataFrame with market features per matchup.
        base_weight: Default weight for base model predictions.
        market_weight: Default weight for market predictions.

    Returns:
        DataFrame with columns: ID, BasePred, MarketPred, BlendedPred, MarketConfidence
    """
    ...
    market_lookup = _build_market_lookup(market_signals)
    ...
    for idx, row in result.iterrows():
        ...
        # Determine blend weights based on confidence
        if confidence >= 0.5:
            # High confidence: use standard weights, adjusted by confidence
            w_base = base_weight - 0.1 * (confidence - 0.5)  # More market as confidence grows
            w_market = 1.0 - w_base
        else:
            # Low confidence: conservative blend
            w_base = CFG.BLEND_LOW_CONFIDENCE_BASE
            w_market = CFG.BLEND_LOW_CONFIDENCE_MARKET

        # Whale signal adjustment: shift toward market when whales are active
        if abs(whale_signal) > 0.5:
    ...
    print(f"  Blended {blended_count}/{len(result)} matchups with market data")
    return result


def _build_market_lookup(market_signals: pd.DataFrame) -> dict[tuple[int, int], dict]:
    """Build a lookup dictionary from market signals.
